...
<?php
 class product
 {
    ...
    public function insert_anhsp($sanpham_id,$sp_anh) {
        ...
        echo "<script> window.location.href='anhsanphamlists.php';</script>";
    ...
    public function insert_sizesp($sanpham_id,$sanpham_size){
        ...
        $result = $this ->db ->insert($query);
        echo "<script> window.location.href='sizesanphamlists.php';</script>";
    ...
    public function insert_product($data,$file){
             ...
             $soluong = $data['soluong'];
              ...
              $query = "INSERT INTO tbl_sanpham (sanpham_tieude,sanpham_ma,danhmuc_id,loaisanpham_id,color_id,sanpham_gia,soluong,sanpham_chitiet,sanpham_baoquan,ngaydang,sanpham_hot,sanpham_anh) 
              VALUES 
              ('$sanpham_tieude','$sanpham_ma','$danhmuc_id','$loaisanpham_id','$color_id','$sanpham_gia','$soluong','$sanpham_chitiet','$sanpham_baoquan','$ngaydang','$sanpham_hot','$sanpham_anh')";
              $result = $this ->db ->insert($query);
              if($result) {
 ...
 public function show_order_xuly() {
    $query = "SELECT tbl_order.*, tbl_payment.*,tbl_diachi.*
    FROM tbl_order INNER JOIN tbl_payment ON tbl_order.session_idA = tbl_payment.session_idA
    INNER JOIN tbl_diachi ON tbl_order.customer_xa = tbl_diachi.ma_px
...
public function show_order_done1(){
    $query = "SELECT tbl_order.*, tbl_payment.*,tbl_diachi.*
    FROM tbl_order INNER JOIN tbl_payment ON tbl_order.session_idA = tbl_payment.session_idA
    INNER JOIN tbl_diachi ON tbl_order.customer_xa = tbl_diachi.ma_px
    WHERE tbl_payment.statusA = 2
    ORDER BY tbl_payment.payment_id DESC   ";
    $result = $this -> db ->select($query);
    return $result;
}
